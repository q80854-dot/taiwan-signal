...
def fetch_ohlcv(ticker: str, tf_key: str = "daily") -> Optional[Dict]:
    ...
    # ── 3. 富果歷史 K 線 ──
    # ★ 修正：2026-08-30——使用者反映全市場掃描（907 檔）一批 50 檔要 3~4 分鐘，
    #   查 log 發現：週線在富果「區間不能超過一年」的限制下（見上面 fubon_broker.py
    #   的修正說明），359 天最多只能拿到約 51 根，永遠到不了下面的 60 根可用門檻——
    #   對週線來說，這個富果請求「打了保證會失敗」，純粹是浪費一次 HTTP 往返
    #   （最長 timeout 10 秒）跟一次免費額度（60次/分鐘），每天 907 檔股票、
    #   每檔都重複發生一次，對整體掃描時間跟富果的分鐘額度都有實際影響。
    #   這裡直接跳過富果、週線一律用 yfinance，省下這個注定用不到的請求。
    #   （日線、60分K 不受影響：日線 359 天能拿到約 250 根，遠超過門檻，是真的
    #   有機會成功、不是浪費；60分K 本來就只抓 90 天上下，同樣沒有這個問題。）
    if FUGLE_API_KEY and tf_key != "weekly":
        ...
        for key, sym in needed.items():
            ...
            try:
                ...
                for fallback_period in ("5d", "1mo", "3mo"):
                    _tkr_try = _yf_ticker(sym)
                    candidate = _tkr_try.history(period=fallback_period, interval="1d", auto_adjust=True)
                    ...
                    if candidate is not None and len(candidate) >= 2 and _last_bar_is_fresh(candidate):
                        h = candidate
                        tkr = _tkr_try  # ★ 保留同一個 Ticker 物件，下面要用它拿 get_history_metadata()
                        break
                ...
                if math.isnan(p) or math.isnan(pv) or p <= 0 or pv <= 0:
                    ...
                    continue
                # ★ 修正：2026-08-30——這裡是使用者回報「加權/S&P500 顯示的收盤都是前一個
                #   交易日」這個問題的根本修正。查證過 yfinance 1.7.0 原始碼(scrapers/history.py)：
                #   history() 打的是 Yahoo 的 /v8/finance/chart/{symbol} 端點，那個回應除了
                #   OHLC 陣列本身，還會附一個 meta 欄位（yfinance 存成 self._history_metadata，
                #   get_history_metadata() 讀的就是這個，不會再打第二次請求，同一次 HTTP 回應
                #   裡的資料），裡面固定有 regularMarketPrice / regularMarketTime 這兩個欄位——
                #   yfinance 自己的原始碼在 history.py 第 1222 行也是拿 regularMarketPrice 去驗證
                #   /修補最後一根 K 線，可見這是 Yahoo 端「即時報價快照」，跟 OHLC 陣列的「最後一根
                #   日K有沒有真的更新」是兩個獨立的欄位。
                #   實測看到的現象（TWII 顯示週四收盤 45,975.22，S&P500 顯示週四 7,730.99 +0.72%，
                #   但週五真實收盤分別是 46,331.45 和 7,711.76 -0.25%）符合「OHLC 陣列的最後一根
                #   還沒更新到最新交易日，但 meta.regularMarketTime 其實已經是最新的」這個情況——
                #   這裡加一層交叉比對：如果 meta 顯示的報價時間比 h 最後一根K的日期還新，代表
                #   history() 回傳的 K 線陣列本身就是舊的，改用 meta.regularMarketPrice 當現價，
                #   原本（其實是舊的）那根當作前收，並清楚記 log，方便之後從正式環境的 log
                #   驗證這個修正是否真的解決問題。
                #   老實說：regularMarketPrice/regularMarketTime 兩個欄位的存在跟用法是直接讀
                #   yfinance 原始碼確認的，不是用猜的；但因為這個 sandbox 連不到 Yahoo，沒辦法
                #   在這裡對一個真的活生生的請求驗證比對邏輯在正式環境是否真的會觸發、觸發後
                #   數字是否真的正確——部署後務必看 log 裡有沒有出現下面這行「偵測到歷史K線
                #   落後」的警告，並拿當天真實收盤數字核對。
                try:
                    meta = tkr.get_history_metadata() or {}
                except Exception as e:
                    meta = {}
                    logger.warning(f"yfinance 大盤 {key}({sym}): get_history_metadata 失敗，略過新鮮度交叉比對：{e}")
                rmp = meta.get("regularMarketPrice")
                rmt = meta.get("regularMarketTime")
                if rmp is not None and rmt is not None:
                    try:
                        rmt_date = rmt.date() if hasattr(rmt, "date") else \
                            datetime.fromtimestamp(int(rmt), tz=timezone.utc).date()
                        last_bar_date = h.index[-1].date() if hasattr(h.index[-1], "date") else None
                        rmp = float(rmp)
                        if last_bar_date is not None and rmt_date > last_bar_date and rmp > 0 \
                                and not math.isnan(rmp):
                            logger.warning(
                                f"yfinance 大盤 {key}({sym}): 偵測到歷史K線落後一個交易日——"
                                f"K線最後一根日期={last_bar_date}（收盤={p}），"
