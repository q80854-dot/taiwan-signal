...
def _fugle_get_candles(ticker: str, code: str, tf_key: str, bars: int) -> Optional[Dict]:
    ...
    try:
        ...
        if d.get("error") or len(closes) < 20:
            return None
...
def _fetch_yf_ohlcv(ticker: str, tf_key: str) -> Optional[Dict]:
    ...
    try:
        ...
        h = None
        for p in periods:
            candidate = yf.Ticker(ticker).history(period=p, interval=tf["interval"], auto_adjust=True)
        ...
        for key, sym in needed.items():
            ...
            try:
                ...
                for fallback_period in ("5d", "1mo", "3mo"):
                    candidate = yf.Ticker(sym).history(period=fallback_period, interval="1d", auto_adjust=True)
