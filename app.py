...
def diagnostics():
    ...
    if fugle_key_set:
        try:
            from fubon_broker import fetch_fugle_market_overview
            _test = fetch_fugle_market_overview()
            fugle_status = "✅ 金鑰已設定且可正常取得資料" if _test else "❌ 金鑰已設定，但呼叫富果 API 沒有拿到資料（可能是金鑰打錯、過期，或額度用完，請到富果開發者後台確認）"
        except Exception as e:
            fugle_status = f"❌ 金鑰已設定，但呼叫富果 API 發生錯誤：{e}"
    return jsonify({
